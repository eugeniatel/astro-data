...
- **Global scope**: `/Users/euge/gachi-pachi/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/euge/gachi-pachi/.claude/notes/`)
...
2. Glob `/Users/euge/gachi-pachi/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/euge/gachi-pachi/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/euge/gachi-pachi/.claude/notes/` — works in any directory
